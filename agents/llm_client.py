...
把这四个检查点前置，后面的讨论才不至于又回到“原则正确、落地很乱”。"""
        ...
        elif "药物基因组学专家" in role or "GWAS专家" in role or "单细胞测序分析师" in role or "Galaxy桥接器" in role:
            return f"""{discussion_bridge}

我从组学与计算分析的角度直接补可执行内容，而不是泛泛介绍技术。

1. 如果这题后面要进入组学分析，第一步先锁定样本/表型定义、采样时间点和主终点，避免实验层与临床层完全脱节。
2. 第二步要把原始数据、元数据和质控日志统一命名，至少要能追溯到受试者编号、时间窗、批次和分析集。
3. 第三步再决定是走 GWAS、药物基因组学、单细胞还是多组学 workflow；不能一开始就把所有算法堆上去。
4. 这类研究真正有价值的地方，在于把生物学信号和临床终点挂钩，而不是只产出一堆差异列表。

我建议下一位由统计学家或临床主任接着收口：先把主终点、协变量和验证路径写死，然后我们再把分析管道接上。"""

        elif "AI数据工程师" in role or "UX研究员" in role:
            return f"""{discussion_bridge}

我从平台执行链路来补这轮最容易被忽略的部分。

1. 先把数据与交互口径统一：研究标题、问题、主终点、时间窗、字段定义必须在前端和导出报告里一致。
2. 再把输入输出拆清：真人一句话需求进来后，系统应该自动生成阶段任务、专家顺序、参考文献和最终交付，不让用户自己在多个页面来回拼。
3. 对于执行体验，我建议保留“随时打断”的权利：用户输入新方向后，应立即停止当前自动接力，并把后续专家排序重排。
4. 最终交付上，要保证聊天记录、阶段清单和报告导出能互相回填，否则平台只是在展示概念。

所以我这边的建议是：每一轮都把结构化状态和最终报告骨架同步维护，这样真人引导和 14 位专家协作才不会脱节。"""
...
站在 {role} 的角度，我建议下一步至少完成四件事：
1. 把本轮最重要的问题缩成一句可验证的研究假设，并明确研究对象与对照边界；
2. 明确主终点和时间窗，避免后面所有角色各说各话；
3. 把这轮应交付的字段、流程步骤和里程碑列成清单，而不是继续停留在原则层；
4. 点名下一位更合适的专家补位，形成真正的接力讨论，而不是单点回答。

如果沿着“{current_request}”这条线继续推进，我建议优先收口主终点、关键风险和下一步交付物，再把我的角色建议嵌进最终报告。"""
