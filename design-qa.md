# Phase 2 comparison + ScientistOne design QA
...
- Desktop viewport: `1280 x 720`; no horizontal overflow in each of the four views.
...
- Navigation: the four side-rail entries switch the active view and URL hash; the original `frontend/index.html` includes both `Phase 2 工作区对比` and `ScientistOne 证据链` entry points.
...
No P0, P1, or P2 visual or functional defects remain in the desktop comparison flow. The ScientistOne metrics are explicitly labelled as paper/reference metrics, not platform guarantees. Future production QA should add real Windows-node API integration tests and a narrow mobile review after the preferred direction is selected.
